...
for(i in seq(length(listGrid))){
  if(length(listGrid[[i]]) == 0){
    ...
    df <- st_drop_geometry(obs[listGrid[[i]],])
    ObsInGridList[[i]] <- df
  }
}
wNonEmpty <- which( lengths(ObsInGridList) > 1)
...
for(i in seq(length(listGrid))){
  if(length(listGrid[[i]]) == 0){
    ...
    df <- st_drop_geometry(obs[listGrid[[i]],])
    ObsInCountyList[[i]] <- df
  }
}
wNonEmpty <- which( lengths(ObsInCountyList) > 1)
