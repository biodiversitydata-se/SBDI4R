...
Similarly, we could search by common names (see Example X for refined taxonomic searches))
```{r, message=FALSE}
...
from one source, e.g. Artportalen. If you see on the data that Artportalens 
...
```
Else, you can search available data resources, collections (and more) using the interactive function \code{pick_filter} like:
```{r, message=FALSE, eval= FALSE}
...
In the same way you can use the spatial layers that are available to spatially search 
for the indexed observations. 
...
xf <- occurrences(taxon="Callitriche cophocarpa", 
                 ...
                 download_reason_id=10)
```
### Other filters on query
Any search could be filtered by any indexed column or variable. You can find which 
are the variables that are indexed writing `sbdi_fields("occurrence_indexed")`. 
These filter strings require SOLR syntax, see [SOLR](https://blog.imaginea.com/solr-query-equivalent-to-the-database-query/)
For example, let's filter out observations with coordinate uncertainty larger than 100 m. 


```{r, message=FALSE}
xf <- occurrences(taxon="Callitriche cophocarpa", 
                 fq="coordinate_uncertainty:[* TO 100]",
                 email="test@test.org", 
                 download_reason_id=10)
...
In the same way, searching for observations between two years would be similar to:
```{r, message=FALSE}
xf <- occurrences(taxon="Callitriche cophocarpa", 
...
You can see a list of all record issues using `sbdi_fields("assertions")` 
and see what is considered as fatal quality issues.
...
```{r grid, fig.width=6, fig.height=6, message=FALSE}
...
```{r counties, fig.width=6, fig.height=6}
palBW <- leaflet::colorNumeric(c("white", "navyblue"), 
                               c(0, max(resSp@data$nObs, na.rm = TRUE)), 
                               na.color = "transparent")
oldpar <- par()
