...
occurrences_plot <- function(x, filename = "Rplots.pdf", 
  ...
  tryCatch({
    if (grouped) {
      tplot(x$data, sweLyr=sweLyr, main="all species", coi=qa, pch, cex)
    } else {
      ...
      for (spp in unique(x$data[, taxon_level])) {
        ...
        if (spp!="") { 
          tplot(x$data[which(x$data[, taxon_level]==spp), ], sweLyr=sweLyr, main=spp, coi=qa, pch, cex)
        } else {
          tplot(x$data[which(x$data[, taxon_level]==spp), ], sweLyr=sweLyr, main="unmatched species name", coi=qa, pch, cex)
        }
      }
    }
